...
class Logger(object):
    ...
    def log(self,
        ...
        if sim:
            self.states[drone, :, current_counter] = np.hstack([state[0:3], state[10:13], state[3:7],state[7:10], state[13:20]])
        else: # This is for real Tello Flights for example... 
            self.states[drone, :, current_counter] = state[:20]
        self.controls[drone, :, current_counter] = control
        self.counters[drone] = current_counter + 1

    ################################################################################

    def save(self, flight_type='simulation'):
        """Save the logs to file.
        """
        with open(os.path.dirname(os.path.abspath(__file__))+"/logs/save-"+flight_type+"-flight-"+datetime.now().strftime("%m.%d.%Y_%H.%M.%S")+".npy", 'wb') as out_file:

...
# For deciding if the point is inside the polygone of buildings...
...
def main():
    ...
    arena_version = 65 # 65 #102

    vehicle_name_list =   ['V1']
    vehicle_source_list = [0.95] # Source_strength
    vehicle_imaginary_source_list = [1.1] # Imaginary source_strength
    ...
    # vehicle_source_list = [1.05, 1.05] # Source_strength
    # vehicle_imaginary_source_list = [1.3, 1.3] # Imaginary source_strength
    # vehicle_goal_list = [([-3.5, 3, 1.4], 5, 0.00), ([-3.5, 3, 1.4], 5, 0.00),]# goal,goal_strength all 5, safety 0.001 for V1 safety = 0 when there are sources
    # vehicle_goto_goal_list =[[1.4,0,0,0], [1.4,0,0,0], ] # altitude,AoA,t_start,Vinf=0.5,0.5,1.5
    # vehicle_pos_list = [[3.5, 3, 1.4], [3.5, 3, 1.4],]
    # vehicle_next_goal_list = [[3.5, 3, 1.4], [-3.5, 3, 1.4]]


    # vehicle_name_list =   ['V1', 'V2', 'V3']
    # vehicle_source_list = [1.05, 1.05, 1.05] # Source_strength
    # vehicle_imaginary_source_list = [1.3, 1.3, 1.3] # Imaginary source_strength
    ...
    try:
        ...
        while time.time()-sim_start_time < 240:
            if time.time()-starttime > 0.025:
                ...
                # Get Target position to follow from Helmet
                # target_position = target_vehicle[0].position

                # Get next pygame event
                k_joyst = 0.05
                pygame.event.pump()
                for j in range(joystick_nr):
                    joyst_target[0] += k_joyst*controller[j].get_axis(0)
                    joyst_target[1] += -k_joyst*controller[j].get_axis(1)
                    joyst_target[2] += 0.01*controller[j].get_axis(2)
                    # joyst_target[j,0] += k_joyst*controller[j].get_axis(0)
                    # joyst_target[j,1] += k_joyst*controller[j].get_axis(1)
                    # joyst_target[j,2] += 0.01*controller[j].get_axis(2)

                # Check if the target point is in a building : for i in range(X.shape[0]): for j in range(Y.shape[0]):
                joyst_target = np.clip(joyst_target,-3.,3.)
                for building in Arena.buildings:
                    if not Point(joyst_target[0], joyst_target[1]).within(Polygon(building.vertices)):
                        target_position = joyst_target.copy()
                    else :
                        print(' Target Inside building !!!!')
                ...
                if use_panel_flow :
                    ...
                    for i, vehicle in enumerate(vehicle_list):
                        ...
                        vehicle.Set_Desired_Velocity(vel_enu, method='None') # 'direct' - 'projection'

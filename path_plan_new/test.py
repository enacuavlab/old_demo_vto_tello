...
def main(arena,telloNet):
  ...
  for ac in tellos_selected:
    ...
    inoutDict[ac]=socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
...
#        sock.sendto(vtupple[0].encode(encoding="utf-8"),telloNet[vtupple[1]][1])
        inoutDict[vtupple[1]].sendto(vtupple[0].encode(encoding="utf-8"),telloNet[vtupple[1]][1])
      else:
        for ac in telloNet:
#          sock.sendto(vtupple[0].encode(encoding="utf-8"),telloNet[ac][1])
          inoutDict[ac].sendto(vtupple[0].encode(encoding="utf-8"),telloNet[ac][1])
